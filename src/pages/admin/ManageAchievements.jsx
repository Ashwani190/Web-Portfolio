          ...
          <PageTransition>
            <div className="max-w-4xl mx-auto">
              ...
              {loading ? <LoadingSpinner /> : data.length === 0 ? (
                <div className="text-center py-20 bg-white rounded-2xl border border-dashed"><Trophy className="mx-auto mb-4 text-canvas/60" size={48} /><p>No achievements added.</p></div>
              ) : (
                ...
                <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed inset-y-0 right-0 w-full sm:w-[450px] bg-silk shadow-xl z-50 flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto p-6">
                    <form id="achieve-form" onSubmit={handleSubmit} className="space-y-4">
                      <input placeholder="Title" required value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="input-field bg-white" />
                      <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="input-field bg-white">
                        {categories.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                      <input type="date" value={formData.date || ''} onChange={e => setFormData({...formData, date: e.target.value})} className="input-field bg-white" />
                  ...
                  <div className="p-6 bg-white flex gap-3"><button type="submit" form="achieve-form" disabled={saving} className="btn-primary w-full">Save</button></div>
                </motion.div>
              </>
            )}
          </AnimatePresence>
