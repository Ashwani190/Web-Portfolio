          ...
          <PageTransition>
            <div className="max-w-4xl mx-auto">
              ...
              {loading ? <LoadingSpinner /> : data.length === 0 ? (
                <div className="text-center py-20 bg-canvas rounded-2xl border border-dashed"><GraduationCap className="mx-auto mb-4 text-canvas/60" size={48} /><p>No education added.</p></div>
              ) : (
                ...
                <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed inset-y-0 right-0 w-full sm:w-[450px] bg-silk shadow-xl z-50 flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto p-6">
                    <form id="edu-form" onSubmit={handleSubmit} className="space-y-4">
                      <input placeholder="Institution" required value={formData.institution} onChange={e => setFormData({...formData, institution: e.target.value})} className="input-field bg-canvas" />
                      <input placeholder="Degree (e.g. B.S. Computer Science)" required value={formData.degree} onChange={e => setFormData({...formData, degree: e.target.value})} className="input-field bg-canvas" />
                      <input placeholder="Field of Study" value={formData.field_of_study || ''} onChange={e => setFormData({...formData, field_of_study: e.target.value})} className="input-field bg-canvas" />
                      <div className="grid grid-cols-2 gap-4">
                        <input type="number" placeholder="Start Year" required value={formData.start_year || ''} onChange={e => setFormData({...formData, start_year: e.target.value})} className="input-field bg-canvas" />
                        <input type="number" placeholder="End Year (Leave blank if present)" value={formData.end_year || ''} onChange={e => setFormData({...formData, end_year: e.target.value})} className="input-field bg-canvas" />
                      </div>
                      <input placeholder="Grade / GPA" value={formData.grade || ''} onChange={e => setFormData({...formData, grade: e.target.value})} className="input-field bg-canvas" />
                  ...
                  <div className="p-6 bg-canvas flex gap-3"><button type="submit" form="edu-form" disabled={saving} className="btn-primary w-full">Save</button></div>
                </motion.div>
              </>
            )}
          </AnimatePresence>
